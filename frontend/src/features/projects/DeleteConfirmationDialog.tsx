...
export function DeleteConfirmationDialog({ open, onOpenChange, projectName = "this project", onConfirm }: DeleteConfirmationDialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <form onSubmit={(e) => { e.preventDefault(); onOpenChange(false); }} className="w-full max-w-sm p-6 bg-white rounded-lg shadow-xl border animate-in fade-in zoom-in duration-200">
        ...
        <div className="flex justify-end gap-2 pt-2">
          ...
          <button type="submit" className="px-4 py-2 bg-delete text-white font-medium rounded-md hover:bg-delete-hover transition-colors">
            Delete
          </button>
        </div>
      </form>
