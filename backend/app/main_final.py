# Sanskriti AI Studio API - Final Main Entry Point with all lyrics CRUD
...
# ============================================
# LYRICS CRUD ENDPOINTS (Global Access)
# ============================================
...
async def search_lyrics(query: str, project_id: str | None = None):
    ...
    if project_id:
        search_query = f"""
            ...
            FROM lyrics l JOIN projects p ON l.project_id = p.id
            WHERE LOWER(l.content) LIKE '%' || LOWER(:query) || '%' AND l.project_id = :project_id
        """
    else:
        search_query = f"""
            SELECT l.id, p.name as project_name, l.title, l.content, l.language, l.status, l.created_at, l.updated_at
