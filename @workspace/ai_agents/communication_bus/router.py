...
class Router:
    ...
    def _ensure_destination_list(self, destination_agent: Union[str, List[str]]) -> List[str]:
        """Convert single agent string or empty to list."""
        if isinstance(destination_agent, str):
            return [destination_agent]
        return list(destination_agent) if destination_agent else []
    ...
    def _is_duplicate(self, message: Any) -> bool:
        ...
        if message_id and message_id in self._last_processed_time:
    ...
    def route(self, message: Any) -> Optional[Any]:
        ...
        destinations = self._ensure_destination_list(getattr(message, 'destination_agent', []))
    ...
    def _route_one_to_one(self, message: Any, destination: str) -> Optional[Any]:
        ...
        for handler in handlers:
            try:
                ...
                if getattr(message, 'retry_count', 0) < self.config.max_retries:
                    message = self._increment_retry(message)
                    continue
                
                # Move to dead-letter queue if enabled
                if self.config.enable_dead_letter_queue:
                    print(f"[ROUTER] Moving to DDLQ: {getattr(message, 'message_id', 'unknown')}")
...
def send_message(
    ...
    return Message.create_request(
        source_agent=source_agent,
        destination_agent=list(destination_agent),  # Ensure list type for create_request
        task_id=task_id,
