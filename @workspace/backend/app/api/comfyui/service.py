...
class ComfyUIManager:
    ...
    def get_response_time(self) -> Optional[float]:
        ...
        return self._response_time_ms if hasattr(self, '_response_time_ms') else 0.0
    ...
    def get_response_time_ms(self) -> float:
        ...
        # Cache response_time to avoid calling get_response_time() multiple times
        rt = self.get_response_time()
        return (rt * 1000 if rt else 0.0)
    ...
    def _count_active_jobs(self, queue_data: Dict) -> int:
        ...
        for item in queue_data.get("queue", []):
            ...
            if prompt and (prompt.get("executing") or prompt.get("pending")):
                count += 1
        return count
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for item in data.get("queue", []):
                    ...
                    if prompt and prompt.get("executing"):
                        job_info = {
                    ...
                    elif prompt and prompt.get("submitted"):
                        job_info = {
                            "id": str(item.get("prompt") or ""),
                            "title": item.get("prompt_data", {}).get("client_id") or "Unknown",
                            "status": "PENDING",
                    ...
                    # Add to completed/failed lists if applicable
                    status = job_info["status"]
                    if status == "COMPLETED" and item.get("prompt"):
                ...
                return {
                    ...
                    "completed": completed_jobs[-10:],  # Last 10 completed
                    "failed": failed_jobs[-10:],        # Last 10 failed
            ...
            if response.status_code == 200:
                ...
                for item in data.get("queue", []):
                    if str(item.get("prompt")) == job_id:
                        ...
                        if prompt and prompt.get("executing"):
                            status = "RUNNING"
                        elif prompt and prompt.get("pending"):
                            status = "PENDING"
                        elif prompt and prompt.get("failed"):
                            status = "FAILED"
                        elif prompt and prompt.get("submitted"):
                            status = "SUBMITTED"
                        else:
    ...
    def submit_workflow(self, workflow: str, inputs: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # Check for redirect (ComfyUI may redirect)
    ...
    def download_output(self, filename: str, save_path: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                ext = os.path.splitext(filename)[1].lower()
                is_image = any(ext in [".png", ".jpg", ".jpeg", ".webp"] for ext in [ext])
                ...
                file_info = {
                    ...
                    "is_image": is_image,
                    "is_video": any(ext in [".mp4", ".mov", ".avi"] for ext in [ext]),
                }
